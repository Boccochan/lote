...
#[async_trait]
impl ChatCompletionClient for ReqwestOllamaClient {
    async fn complete(
        &self,
        model: &str,
        ...
        loop {
            ...
            if !status.is_success() {
                ...
                if status.as_u16() == 400 && send_tools && ollama_tools_unsupported(&txt) {
                    log::warn!(
                        "[ollama] model {model:?}: server does not support tools; retrying without tools (tool calls will not work until you use a tool-capable model)"
                    );
